...
*/
template <typename Dtype>
class BinWriteLayer : public Layer<Dtype> {
 public:
  explicit BinWriteLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const { return "BinWrite"; }
  virtual inline int ExactNumTopBlobs() const { return 0; }
 ...
 protected:
  ...
  virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  virtual void WriteBlobToBin(const char *fname, Blob<Dtype>* blob);
...
/**
 * @brief Resize the input feature map by a factor of resize_ratio.
 *
 * TODO(dox): thorough documentation for Forward, Backward, and proto params.
 */
template <typename Dtype>
...
/**
 * @brief Upsamples the input feature map by a factor of upsample_ratio.
 *
 * TODO(dox): thorough documentation for Forward, Backward, and proto params.
 */
template <typename Dtype>
...
class SpatialPoolingLayer : public Layer<Dtype> {
 public:
  ...
  // others can only output the pooled inputs.
  virtual inline int MaxTopBlobs() const {
    return (this->layer_param_.spatial_pooling_param().pool() ==
            SpatialPoolingParameter_PoolMethod_MAX) ? 2 : 1;
  }
