function a=read_annotations(root_dir)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
...
% %         root_dir = [fileparts(root_dir) '/info'];
    ...
    for i=1:length(files)
        ...
        for kp=0:(kpNodes.getLength-1)
            ...
            if ~isscalar(kpid)
                if strcmp(name, 'L_Foot')
                    kpid = 18;
                elseif strcmp(name, 'R_Foot')
                    kpid = 19;
                elseif strcmp(name, 'HeadBack')
                    kpid = 20;
                else
        ...
        a.segment_coords{i}=uint16([]);
        a.segment_labels{i}=uint8([]);
        segNodes = docRootNode.getElementsByTagName('polylinesegments').item(0);
        if ~isempty(segNodes)
...
% %                 end
                seg_coord_x = str2num(segNodes.item(kp).getAttribute('x').toCharArray');
                seg_coord_y = str2num(segNodes.item(kp).getAttribute('y').toCharArray');
                a.segment_coords{i} = uint16([seg_coord_x' seg_coord_y']);
                
                a.segment_labels{i} = category_id;
            end
        end
        % disp(files{i});
    end
    ...
    if any(a.entry_id==-1)
        ...
        disp('Warning! Some entries have no entry_id.');
    end
