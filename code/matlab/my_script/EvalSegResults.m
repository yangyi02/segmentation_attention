...
if is_server
  if strcmp(dataset, 'voc12') || strcmp(dataset, 'voc10_part') || strcmp(dataset, 'voc10_person')
  ...
  elseif strcmp(dataset, 'cloth_data_6')
    VOC_root_folder = fullfile(WORK_ROOT_DIR, 'rmt/data/cloth_data_6');    
  else
    error('Wrong dataset');
  end
else
...
output_mat_folder = fullfile(WORK_ROOT_DIR, '/rmt/work/deeplabel_baidu/exper', dataset, feature_name, model_name, testset, feature_type);
...
elseif strcmp(dataset, 'voc10_person')
  seg_res_dir = [save_root_folder '/results/VOC2010_person/'];
  seg_root    = fullfile(VOC_root_folder, 'VOC2012');
  gt_dir      = fullfile(VOC_root_folder, 'VOC2012', 'SegmentationPerson');  
elseif strcmp(dataset, 'cloth_data_6')
  seg_res_dir = [save_root_folder, '/results/cloth_data_6/'];
  seg_root    = fullfile(VOC_root_folder, '');
  gt_dir      = fullfile(VOC_root_folder, '', 'SegmentationClass');  
end
...
elseif strcmp(dataset, 'coco')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'coco');
elseif strcmp(dataset, 'voc10_part')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'VOC2010_Part');
elseif strcmp(dataset, 'voc10_person')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'VOC2010_Person');
elseif strcmp(dataset, 'cloth_data_6')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'cloth_data_6');
end
...
if is_mat
  ...
  for i = 1 : numel(output_dir)
    ...
    elseif strcmp(dataset, 'coco') || strcmp(dataset, 'cloth_data_6')   
