...
V1LayerParameter_LayerType UpgradeV0LayerType(const string& type) {
  if (type == "accuracy") {
    ...
    return V1LayerParameter_LayerType_BATCHNORM;
  } else if (type == "bn") {
...
const char* UpgradeV1LayerType(const V1LayerParameter_LayerType type) {
  ...
  case V1LayerParameter_LayerType_ARGMAX:
    return "ArgMax";
  case V1LayerParameter_LayerType_BATCHNORM:
    return "BatchNorm";
  ...
  case V1LayerParameter_LayerType_SIGMOID_WEIGHTED_CROSS_ENTROPY_LOSS:
    return "SigmoidWeightedCrossEntropyLoss";
